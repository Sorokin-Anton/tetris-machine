...
step :: Model -> Model
step model =
  let TetrisConfig {boardSize = (sizeX, sizeY)} = modelConfig model
   in model
        { modelPic =
            M.fromList
              ( catMaybes
                  [ case modelPic model !? (i, j) of
                      Just cellColor ->
                        if length (catMaybes (map (modelPic model !?) (neighbours (i, j)))) `elem` [2, 3]
                          then Just ((i, j), cellColor)
                          else Nothing
                      Nothing ->
                        if length (catMaybes (map (modelPic model !?) (neighbours (i, j)))) == 3
